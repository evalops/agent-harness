...
from agent_harness import ToolRegistry, get_registry, python_type_to_json_schema, register_tool
...
def test_registry_thread_safety():
    ...
    def register_tools(prefix: str):
        for i in range(10):
            registry.register(f"{prefix}_{i}", f"Tool {prefix} {i}", lambda x: x)

    threads = [threading.Thread(target=register_tools, args=(f"thread{i}",)) for i in range(5)]
